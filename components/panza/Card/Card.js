...
import { Base, Image, Text, config } from "../index";
...
export const CardHeader = ({ avatar, title, subtitle, children, ...other }) => (
  <Base flexDirection="row" p={2} {...other}>
    ...
    <Base flex={1}>
      {title && (typeof title === "string" ? <Text bold>{title}</Text> : title)}
      {subtitle &&
...
export const Card = ({ children, ...other }, { panza }) => {
